...
--- a/Cal_DailyISOP_FromHCHO_Multipleregression_LSTM_anofromMatlab.py
+++ b/Cal_DailyISOP_FromHCHO_Multipleregression_LSTM_anofromMatlab.py
@@ -1,1182 +1,1185 @@
-
-# -*- coding: utf-8 -*-
-"""
...
-# ↑↑↑ 特征选择配置 ↑↑↑
-
-SAVE_DIR = r"E:\2026\Result0_ISOP\LSTM_Daily_Results"
...
-print(f"验证集:   固定最后 {VAL_STEPS} 个序列（共78序列，训练{78-VAL_STEPS}/验证{VAL_STEPS}）")
...
-print("\n[1/5] 读取数据...")
-
-ROW_RANGE = [268, 313]   # 数组行索引范围 lat
-COL_RANGE = [213, 258]   # 数组列索引范围
-
...
-    arr = da_slice.values.astype(np.float32).reshape(7*365, 45,45)
-    ds.close()
-    
-    return arr
-
...
-    
-    # 计算能生成的最大序列数
-    max_seqs = max(0, n_valid_samples - seq_len + 1)
...
-    if finite_ratio < 0.3:  # 要求至少30%有效数据点
-        train_fail_nan += 1
-        pbar.update(1)
-        continue
-
...
-    # 检查训练是否成功
-    if model is None:
-        if len(metrics) == 0:  # metrics 为空字典表示样本不足或其他原因
-            train_fail_sample += 1
-        else:
