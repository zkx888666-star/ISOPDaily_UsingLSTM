...
# -*- coding: utf-8 -*-
"""
Created on Mon Apr 13 15:33:56 2026
...
# -*- coding: utf-8 -*-
"""
Created on Tue Mar  3 09:16:08 2026

@author: 52464
V4
Circulation组也改为每组Top-2（共选12个），减少参数；
删除采样阶段；
调整：验证集改为最后 n 个固定步数而非比例，确保验证集够用；
降低最小样本门槛。
"""
# =============================================================================
# LSTM 逐像元回归 + 特征重要性排序
...
#训练配置: EPOCHS=200, BATCH_SIZE=16
#学习率:   初始=0.001, 最小=1e-05
#验证集:   固定最后 12 个序列（共78序列，训练66/验证12）
#特征选择: 每组Top-2，共6组×2=12个特征
# =============================================================================
import os
import numpy as np
import xarray as xr
import warnings, sys, multiprocessing as mp
...
# ↓↓↓ 可调训练超参数 ↓↓↓
EPOCHS      = 500        # 训练轮次：建议 100~500
BATCH_SIZE  = 64        # 批次大小：建议 8~64
LR_INIT     = 1e-4       # Adam 初始学习率
LR_MIN      = 1e-5       # 学习率下限
LR_FACTOR   = 0.5        # 学习率衰减因子
LR_PATIENCE = 70         # 触发衰减前的耐心轮数
PIXEL_BATCH_SIZE = 16     # Pixel batch 训练：每批处理的像元数量
PARALLEL_WORKERS = 4      # 并行训练线程数（CPU建议>1；GPU建议1）
# ↑↑↑ 可调训练超参数 ↑↑↑

# ↓↓↓ 验证集 & 交叉验证配置 ↓↓↓
# 验证集使用固定步数（而非比例），避免小样本时验证集过少导致早停失效
# T=84, SEQ_LEN=6 → 有效序列78步；VAL_STEPS=12 → 训练66步/验证12步
VAL_STEPS          = 360    # 验证集固定步数（最后N个序列）
USE_KFOLD          = False  # 全图模式下关闭K折（速度优先）；如需评估可设True
KFOLD_N_SPLITS     = 3      # K折数（USE_KFOLD=True时生效）
# ↑↑↑ 验证集 & 交叉验证配置 ↑↑↑

# ↓↓↓ 特征选择配置 ↓↓↓
GROUP_TOP_K    = 5           # 每组保留 Top-K 个特征（所有组统一，共6组×2=12个）
FULL_SEL_GROUP = None        # 不再全选任何组（设为None）
PRESELECT_MODE = "hybrid"    # 打分方式：hybrid / correlation / mutual_info
# ↑↑↑ 特征选择配置 ↑↑↑

SAVE_DIR = r"E:\2026\Result0_ISOP\LSTM_Daily_Results_V1_AddPara_SEQ150HID128"
os.makedirs(SAVE_DIR, exist_ok=True)

print(f"Device: {DEVICE}")
print(f"PyTorch: {torch.__version__}")
print(f"训练配置: EPOCHS={EPOCHS}, BATCH_SIZE={BATCH_SIZE}, PIXEL_BATCH_SIZE={PIXEL_BATCH_SIZE}")
print(f"学习率:   初始={LR_INIT}, 最小={LR_MIN}")
T= 2555
total_seq_hint = max(0, T - SEQ_LEN)
train_seq_hint = max(0, total_seq_hint - min(VAL_STEPS, total_seq_hint))
val_seq_hint = min(VAL_STEPS, total_seq_hint)
print(f"验证集:   固定最后 {VAL_STEPS} 个序列（理论总序列{total_seq_hint}，训练{train_seq_hint}/验证{val_seq_hint}）")
print(f"特征选择: 每组Top-{GROUP_TOP_K}，共6组×{GROUP_TOP_K}=12个特征")
...
ROW_RANGE = [268, 283]   # 数组行索引范围 lat
COL_RANGE = [243, 258]   # 数组列索引范围
...
import h5py
import numpy as np
file_path = r"E:/2026/Result0_ISOP/ISOP_TrendandWhy/MTCO2_dailyano_2018T2024_FromECWMF.mat"
with h5py.File(file_path, 'r') as f:
    # 查看文件中的所有变量
    print("文件中的变量:", list(f.keys()))
    MTCO2 = f['mtco2_T_dailyano3D'][:,COL_RANGE[0]:COL_RANGE[1],ROW_RANGE[0]:ROW_RANGE[1]]  
    Lon_MTCO2 = f['Lon_T'][:] 
    Lat_MTCO2 = f['Lat_T'][:] 
    

MTCO2_2 = np.expand_dims(MTCO2, axis=1)

##
BASE_DIR = r"E:\2026\Result0_ISOP\ISOP_TrendandWhy\data\Daily_Anomaly"

# 每个参数对应的 nc 文件路径
NC_FILES = {
    "HCHO": os.path.join(BASE_DIR, "HCHO_Anomaly2018T2025.nc"),
    "ISOP": os.path.join(BASE_DIR, "ISOP_Anomaly2018T2025.nc"),
    "LAI":  os.path.join(BASE_DIR, "LAI_Anomaly2018T2025.nc"),
    "NDVI": os.path.join(BASE_DIR, "NDVI_Anomaly2018T2025.nc"),
    "O3":   os.path.join(BASE_DIR, "O3_Anomaly2018T2025.nc"),
    "PR":   os.path.join(BASE_DIR, "PR_Anomaly2018T2025.nc"),
    "RAD":  os.path.join(BASE_DIR, "RAD_Anomaly2018T2025.nc"),
    "ST01": os.path.join(BASE_DIR, "ST01_Anomaly2018T2025.nc"),
    "ST02": os.path.join(BASE_DIR, "ST02_Anomaly2018T2025.nc"),
    "ST03": os.path.join(BASE_DIR, "ST03_Anomaly2018T2025.nc"),
    "ST04": os.path.join(BASE_DIR, "ST04_Anomaly2018T2025.nc"),
    "SW01": os.path.join(BASE_DIR, "SW01_Anomaly2018T2025.nc"),
    "SW02": os.path.join(BASE_DIR, "SW02_Anomaly2018T2025.nc"),
    "SW03": os.path.join(BASE_DIR, "SW03_Anomaly2018T2025.nc"),
    "SW04": os.path.join(BASE_DIR, "SW04_Anomaly2018T2025.nc"),
    "T2m":  os.path.join(BASE_DIR, "T2m_Anomaly2018T2025.nc"),
    "CLD":  os.path.join(BASE_DIR, "CLD_Anomaly2018T2025.nc")
}

T   = 365*7

# ====== 函数：按切片读取 NetCDF 数据 ======
def load_nc_sliced(file_path, row_range, col_range):
    """
    读取 NetCDF 并只截取指定行列范围，返回 (time, rows, cols) ndarray
    """
    ...
    # 检查维度顺序，通常 (time, lat, lon)
    da = ds[var_name]
    
    # 截取行列
    da_slice = da.isel(lat=slice(row_range[0], row_range[1]),
                       lon=slice(col_range[0], col_range[1]))
    da_slice = da_slice[0:7,:,:,:]
    arr = da_slice.values.astype(np.float32).reshape(7*365, 15,15)
    ds.close()
    
    return arr


ISOP = load_nc_sliced(NC_FILES["ISOP"], ROW_RANGE, COL_RANGE)
rows, cols, T_len = ISOP.shape[1], ISOP.shape[2], ISOP.shape[0]
X_FEATURES = ["T2m", "ST01", "ST02", "ST03", "ST04", "SW01", "SW02", "SW03", "SW04",                     
"LAI", "NDVI","O3","RAD","PR", "CLD"]
Xall = np.stack([load_nc_sliced(NC_FILES[v], ROW_RANGE, COL_RANGE) for v in X_FEATURES], axis=1)

ds = xr.open_dataset(NC_FILES["ISOP"])
lon = ds['Lon'].isel(lat=slice(ROW_RANGE[0], ROW_RANGE[1]),
                   lon=slice(COL_RANGE[0], COL_RANGE[1])).values.astype(np.float32)
lat = ds['Lat'].isel(lat=slice(ROW_RANGE[0], ROW_RANGE[1]),
                   lon=slice(COL_RANGE[0], COL_RANGE[1])).values.astype(np.float32)
lon_1d = ds['lon'].isel( lon=slice(COL_RANGE[0], COL_RANGE[1])).values.astype(np.float32)
lat_1d = ds['lat'].isel(lat=slice(ROW_RANGE[0], ROW_RANGE[1])).values.astype(np.float32)
# 环流场数据
...
targets = []
for f in nc_files:
    targets.append(read_1d_nc_to_3d(f, (T , rows, cols)))
Cirs = np.stack(targets, axis=1)

X_full = np.concatenate((Xall, MTCO2_2), axis=1)
...
GROUP_TOP_K    = 5       
# 所有组统一 Top-GROUP_TOP_K，无需全选
PRESELECT_MODE = "hybrid"   # 可选: "hybrid" | "correlation" | "mutual_info"

def preselect_pixel(y_ts, X_ts_norm, groups, mode=PRESELECT_MODE):
    """
    对单格点做组内特征预筛选：所有组均取 Top-GROUP_TOP_K。
    6组 × 2 = 12个特征输入LSTM。
    y_ts      : [T]     目标（ISOP原始值）
    X_ts_norm : [T, 22] 已全局归一化的特征
    返回      : list of int，选中的全局特征索引（已排序），失败返回None
    """
    mask = np.isfinite(y_ts) & np.isfinite(X_ts_norm).all(axis=1)
    if mask.sum() < 5:  # 降低到5个有限值即可尝试筛选
        return None

    yv = y_ts[mask]
    Xv = X_ts_norm[mask]
    ys = (yv - yv.mean()) / (yv.std() + 1e-8)   # 局部标准化用于打分

    selected = []
    for gname, idx in groups.items():
        idx  = list(idx)
        Xg   = Xv[:, idx]
        keep = min(GROUP_TOP_K, len(idx))   # 不超过该组实际特征数

        if mode == "hybrid":
            corr_mat = np.corrcoef(Xg.T, ys)
            corr_s   = np.abs(np.nan_to_num(corr_mat[:-1, -1], 0))
...
# 每个像元有效比例
isop_ratio = np.isfinite(ISOP).sum(axis=0) / ISOP.shape[0]

# X：要求所有变量同时有效（逐时间）
x_valid_time = np.isfinite(X_norm).all(axis=1)   # [T, rows, cols]
x_ratio = x_valid_time.sum(axis=0) / X_norm.shape[0]

#
valid_mask = (isop_ratio > 0.1) & (x_ratio > 0.1)
valid_pix  = np.where(valid_mask.ravel())[0]
...
class LightLSTM(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim=HIDDEN_DIM, num_layers=1, dropout=0.2):
        super().__init__()
        self.lstm = nn.LSTM(
...
class PixelLSTMWrapper(BaseEstimator, RegressorMixin):
    """
    sklearn 兼容的 LSTM 包装器。
    注意：sklearn 的 permutation_importance 要求 estimator 已被 fit，
    因此提供一个空的 fit 方法并设置 _is_fitted 标志。
    """
...
def permutation_importance_manual(wrapper, X, y, n_repeats=5, random_state=42):
    """
    手动实现排列重要性，完全绕开 sklearn estimator 验证。
    X : [T, K]（已标准化）
    y : [T]（原始值）
    返回 importances_mean [K]
    """
    rng      = np.random.default_rng(random_state)
    baseline = wrapper.score(X, y)
    K        = X.shape[1]
    scores   = np.zeros((n_repeats, K))

    for r in range(n_repeats):
        for k in range(K):
            X_perm      = X.copy()
            X_perm[:, k] = rng.permutation(X_perm[:, k])
            scores[r, k] = baseline - wrapper.score(X_perm, y)

    return np.maximum(scores.mean(axis=0), 0)   # 负值（打乱反而更好）截断为0
...
def train_pixel(x_ts, y_ts, seq_len=SEQ_LEN, epochs=EPOCHS,
                device=DEVICE, use_kfold=USE_KFOLD):
    """
    逐像元LSTM训练。
    x_ts : [T, K]  特征时间序列（已全局归一化）
    y_ts : [T]     ISOP时间序列（原始尺度）
    验证集 = 最后 VAL_STEPS 个序列或样本总数的10~20%（灵活设置）
    """
    ...
    # 动态验证集大小：根据样本数调整，但至少3个序列、最多VAL_STEPS个
    n_valid_samples = len(y_ts)
    if n_valid_samples < seq_len + 8:  # 最低要求：能生成 4 个训练序列 + 4 个验证序列
        return None, StandardScaler(), StandardScaler(), {}
    
    # 计算可生成的监督样本数（t 从 seq_len 到 n_valid_samples-1，共 n_valid_samples-seq_len 个）
    max_seqs = max(0, n_valid_samples - seq_len)
    # 验证集大小：min(VAL_STEPS, max_seqs//4)，即训练数据的25%左右
    n_val_seqs = max(3, min(VAL_STEPS, max(4, max_seqs // 4)))
    ...
    if len(Xs) < 8:  # 最少8个序列：4个训练 + 4个验证
        return None, sc_x, sc_y, {}



    # ==================================================================
    # 策略 A：时序K折（USE_KFOLD=True，每折验证集大小=n_val_seqs）
    # ==================================================================
    if use_kfold and KFOLD_N_SPLITS >= 2:
        if len(Xs) < KFOLD_N_SPLITS * (n_val_seqs + 2):
            use_kfold = False   # 样本不够分折，退回单次

    if use_kfold:
        tscv         = TimeSeriesSplit(n_splits=KFOLD_N_SPLITS,
                                       test_size=n_val_seqs)
        fold_metrics = []
        for fold_tr_idx, fold_va_idx in tscv.split(Xs):
            if len(fold_tr_idx) < 4 or len(fold_va_idx) < 2:
                continue
            Xtr = Xs[fold_tr_idx].astype(np.float32)
            Ytr = Ys[fold_tr_idx].astype(np.float32)
            Xva = Xs[fold_va_idx].astype(np.float32)
            Yva = Ys[fold_va_idx].astype(np.float32)
            fm, _ = _train_one_fold(Xtr, Ytr, Xva, Yva, K, epochs, device)
            fm.eval()
            with torch.no_grad():
                pn = fm(torch.from_numpy(Xva).to(device)).cpu().numpy()
            pred = sc_y.inverse_transform(pn.reshape(-1,1)).ravel()
            true = sc_y.inverse_transform(Yva.reshape(-1,1)).ravel()
        ...
        # 最终模型用固定分割重训
        split_pt = len(Xs) - n_val_seqs
        X_tr_base = Xs[:split_pt].astype(np.float32)
        Y_tr_base = Ys[:split_pt].astype(np.float32)
        X_va_base = Xs[split_pt:].astype(np.float32)
        Y_va_base = Ys[split_pt:].astype(np.float32)
        model, _ = _train_one_fold(X_tr_base, Y_tr_base,
                                   X_va_base, Y_va_base, K, epochs, device)

    # ==================================================================
    # 策略 B：单次固定分割（默认，全图训练用此路径）
    # ==================================================================
    else:
        split_pt = len(Xs) - n_val_seqs
        X_tr_base = Xs[:split_pt].astype(np.float32)
        ...
        model.eval()
        with torch.no_grad():
            pn = model(torch.from_numpy(Xs.astype(np.float32)).to(device)).cpu().numpy()
        pred    = sc_y.inverse_transform(pn.reshape(-1,1)).ravel()
...
def _train_single_pixel_task(pidx):
    ri, ci_ = divmod(int(pidx), cols)
    feat_idx = PIX_FEAT_IDX[pidx]
    if feat_idx is None:
        return dict(status="fail_feat", pidx=pidx)

    x_ts  = X_norm[:, feat_idx, :][:, :, ri, ci_]
    y_ts  = ISOP[:, ri, ci_]
    finite_ratio = np.isfinite(x_ts).all(axis=1).sum() / len(y_ts)
    if finite_ratio < 0.3:
        return dict(status="fail_nan", pidx=pidx)

    model, sc_x, sc_y, metrics = train_pixel(x_ts, y_ts, use_kfold=False)
    if model is None:
        return dict(status="fail_sample", pidx=pidx)

    return dict(status="ok", ri=ri, ci=ci_, feat_idx=feat_idx,
                model=model, sc_x=sc_x, sc_y=sc_y, metrics=metrics, x_ts=x_ts, y_ts=y_ts)

# =============================================================================
# 5. 全图逐像元训练
# =============================================================================
print("\n[3/5] 全图逐像元训练（逐像元自适应特征 + 三种重要性 + 预测值）...")
...
# 统计训练结果
train_success = 0
train_fail_feat = 0      # 特征选择失败
train_fail_sample = 0    # 样本不足
train_fail_training = 0  # 训练失败
train_fail_nan = 0       # NaN数据过多

for b0 in range(0, len(valid_pix), PIXEL_BATCH_SIZE):
    pix_batch = valid_pix[b0:b0 + PIXEL_BATCH_SIZE]
    if DEVICE.type == 'cuda' or PARALLEL_WORKERS <= 1:
        batch_results = [_train_single_pixel_task(pidx) for pidx in pix_batch]
    else:
        with ThreadPoolExecutor(max_workers=PARALLEL_WORKERS) as ex:
            batch_results = list(ex.map(_train_single_pixel_task, pix_batch))

    for res in batch_results:
        if res['status'] == 'fail_feat':
            train_fail_feat += 1
            pbar.update(1)
            continue
        if res['status'] == 'fail_nan':
            train_fail_nan += 1
            pbar.update(1)
            continue
        if res['status'] == 'fail_sample':
            train_fail_sample += 1
            pbar.update(1)
            continue

        ri, ci_ = res['ri'], res['ci']
        feat_idx = res['feat_idx']
        K_pix = len(feat_idx)
        model, sc_x, sc_y = res['model'], res['sc_x'], res['sc_y']
        metrics, x_ts, y_ts = res['metrics'], res['x_ts'], res['y_ts']
    
        # 训练成功，保存指标
        if metrics:
            R2_map[ri, ci_]    = metrics.get('r2', np.nan)
            RMSE_map[ri, ci_]  = metrics.get('rmse', np.nan)
            MAE_map[ri, ci_]   = metrics.get('mae', np.nan)
            MBE_map[ri, ci_]   = metrics.get('mbe', np.nan)
            RRMSE_map[ri, ci_] = metrics.get('rrmse', np.nan)
            CORR_map[ri, ci_]  = metrics.get('corr', np.nan)
            KGE_map[ri, ci_]   = metrics.get('kge', np.nan)
            train_success += 1
        else:
            train_fail_training += 1
        ...
        # ---- 重要性2：Permutation（手动实现）----
        try:
            wrapper = PixelLSTMWrapper(model, SEQ_LEN, sc_x, sc_y, DEVICE).fit(X_n, y_ts)
            perm_n  = permutation_importance_manual(wrapper, X_n, y_ts,
...
# ═══════════════════════════════════════════════════════════════════════════
print(f"\n  ✓ 全图训练统计:")
print(f"    总有效格点数    : {len(valid_pix)}")
print(f"    训练成功        : {train_success} ({100*train_success/len(valid_pix):.1f}%)")
print(f"    特征选择失败    : {train_fail_feat} ({100*train_fail_feat/len(valid_pix):.1f}%)")
print(f"    NaN 数据过多    : {train_fail_nan} ({100*train_fail_nan/len(valid_pix):.1f}%)")
print(f"    样本不足        : {train_fail_sample} ({100*train_fail_sample/len(valid_pix):.1f}%)")
print(f"    LSTM 训练失败   : {train_fail_training} ({100*train_fail_training/len(valid_pix):.1f}%)")
if train_success > 0:
    print(f"\n  全图 R² 均值 (成功格点): {np.nanmean(R2_map[~np.isnan(R2_map)]):.3f}")
    print(f"  R² 空间覆盖率         : {(~np.isnan(R2_map)).sum() / (rows*cols) * 100:.1f}%")
...
# 坐标轴
lon_1d_coord = lon_1d.astype(np.float32)
lat_1d_coord = lat_1d.astype(np.float32)
...
print("  feature_importance_ranking.csv       — 三方法重要性排名表（全图格点均值）")
